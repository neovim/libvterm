...
{
  cb_count = 0;
  vterm_push_bytes(vt, "\xC2\x80\xDF\xBF", 4);
...
{
  cb_count = 0;
  vterm_push_bytes(vt, "\xE0\xA0\x80\xEF\xBF\xBD", 6);
...
{
  cb_count = 0;
  vterm_push_bytes(vt, "\xF0\x90\x80\x80\xF7\xBF\xBF\xBF", 8);
...
static void test_earlyterm(void)
{
  ...
  // Three byte
  cb_count = 0;
  vterm_push_bytes(vt, "\xE0!\xE0\xA0!", 5);
  ...
  // Four byte
  cb_count = 0;
  vterm_push_bytes(vt, "\xF0!\xF0\x90!\xF0\x90\x80!", 9);
...
static void test_earlyrestart(void)
{
  // Two byte
  cb_count = 0;
  vterm_push_bytes(vt, "\xC2\xC2\x90", 3);
  ...
  // Three byte
  cb_count = 0;
  vterm_push_bytes(vt, "\xE0\xC2\x90\xE0\xA0\xC2\x90", 7);
  ...
  // Four byte
  cb_count = 0;
  vterm_push_bytes(vt, "\xF0\xC2\x90\xF0\x90\xC2\x90\xF0\x90\x80\xC2\x90", 12);
...
static void test_overlong(void)
{
  // Two byte
  cb_count = 0;
  vterm_push_bytes(vt, "\xC0\x80\xC1\xBF", 4);
  ...
  // Three byte
  cb_count = 0;
  vterm_push_bytes(vt, "\xE0\x80\x80\xE0\x9F\xBF", 6);
  ...
  // Four byte
  cb_count = 0;
  vterm_push_bytes(vt, "\xF0\x80\x80\x80\xF0\x8F\xBF\xBF", 8);
...
static void test_utf16surr(void)
{
  ...
  cb_count = 0;
  vterm_push_bytes(vt, "\xED\xA0\x80\xED\xBF\xBF", 6);
