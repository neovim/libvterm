...
void ecma48_input_push_str(ecma48_t *e48, ecma48_mod_e state, char *str, size_t len)
{
  ...
  if(state_noshift == 0)
    ...
    ecma48_push_bytes_output(e48, str, len);
  else if (state_noshift == ECMA48_MOD_CTRL && len == 1) {
    // Ctrl + normal symbol
    char c = str[0] & 0x1f;
    ecma48_push_bytes_output(e48, &c, 1);
  }
  else {
...
void ecma48_input_push_key(ecma48_t *e48, ecma48_mod_e state, ecma48_key_e key)
{
  ...
  case ECMA48_KEY_ENTER:
    ecma48_push_bytes_output(e48, "\n", 1); break;
  case ECMA48_KEY_TAB:
    ecma48_push_bytes_output(e48, "\t", 1); break;
  case ECMA48_KEY_BACKSPACE:
    ecma48_push_bytes_output(e48, "\b", 1); break;
  case ECMA48_KEY_ESCAPE:
    ecma48_push_bytes_output(e48, "\e", 1); break;
  default:
